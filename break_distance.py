...
def removeDuplicates(blocks_p,blocks_q):
    ...
    for id,pos in enumerate(blocks_p):
        if abs(pos) not in used_p and abs(blocks_q[id]) not in used_q:
            used_p.add(abs(pos))
            used_q.add(abs(blocks_q[id]))
...
def run(plasmidA,plasmidB):
    ...
    # print()
    penal=1
    dist = penal*indel_distance+calculate2BreakDistance(genomes[0], genomes[1])
    return dist
